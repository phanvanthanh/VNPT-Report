...
<table class="table table-hover table-bordered" cellspacing="0" width="100%">'
    <thead>
        <tr>
            ...
            <th class="text-center">Loại XL</th>
    ...
    <tbody>
        ...
        @foreach($dsCongViecs as $congViec)
            ...
            <tr style="cursor: pointer;" data="{{$congViec->id}}" class="js-xem-cay-xu-ly <?php if($stt==1){echo 'active';} ?>">
                ...
                <td>
                    @if($congViec->la_xu_ly_chinh==0)
                        <span class="label label-warning">HT</span>
                    @else
                        <span class="label label-danger">XLC</span>
                    @endif
                </td>
                <td class="text-center">
                        ...
                        @if($congViec->la_xu_ly_chinh==1)
                            <button type="button" data="{{$congViec->id}}"  data-toggle="modal" data-target=".bs-yc-ho-tro-modal-lg" class="btn btn-gradient waves-effect waves-light js-yc-ho-tro"  data-toggle="tooltip" title="Yêu cầu lãnh đạo hỗ trợ"><i class="dripicons-reply-all"></i></button>
                            <button type="button" data="{{$congViec->id}}"  data-toggle="modal" data-target=".bs-chuyen-lanh-dao-duyet-modal-lg" class="btn btn-gradient waves-effect waves-light js-chuyen-ld-duyet"  data-toggle="tooltip" title="Chuyển lãnh đạo duyệt">
                                <i class="dripicons-forward"></i>
                            </button>
                        @else
                            @if($congViec->xu_ly==1) 
                                <button type="button" data="{{$congViec->id}}" class="btn btn-success waves-effect waves-light js-bao-cao-hoan-tat">
                                    <i class="dripicons-retweet" data-toggle="tooltip" title="Đã hoàn tất hỗ trợ"></i>
                                </button>
                            @else 
                                <button type="button" data="{{$congViec->id}}" class="btn btn-warning waves-effect waves-light js-bao-cao-hoan-tat" data-toggle="tooltip" title="Báo cáo hoàn tất việc hỗ trợ">
...
<div class="modal fade bs-yc-ho-tro-modal-lg" tabindex="-1" role="dialog" aria-labelledby="modalYcHoTro" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-lg center">
        <form class="form-horizontal" role="form" name="frm_yeu_cau_ho_tro" id="frm_yeu_cau_ho_tro">
        {{ csrf_field() }}
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h5 class="modal-title" id="myLargeModalLabel">YÊU CẦU HỖ TRỢ</h5>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-xs-12 col-md-12">
                        <input type="hidden" name="id_cong_viec" class="id-cong-viec">
                        <label for="noi_dung_yeu_cau">Nội dung yêu cầu hỗ trợ <span class="text-danger"></span></label>
                        <textarea rows="5" class="form-control noi-dung-yeu-cau" name="noi_dung_yeu_cau" id="noi_dung_yeu_cau"></textarea>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary btn-yeu-cau-ho-tro"><i class="dripicons-alarm"></i> Gửi yêu cầu</button>
...
<div class="modal fade bs-chuyen-lanh-dao-duyet-modal-lg" tabindex="-1" role="dialog" aria-labelledby="modalChuyenLanhDaoDuyet" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-lg center">
        <form class="form-horizontal" role="form" name="frm_chuyen_lanh_dao_duyet" id="frm_chuyen_lanh_dao_duyet">
        {{ csrf_field() }}
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h5 class="modal-title" id="myLargeModalLabel">CHUYỂN LÃNH ĐẠO DUYỆT</h5>
            </div>
            <div class="modal-body">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary btn-chuyen-lanh-dao"><i class="dripicons-alarm"></i> Chuyển lãnh đạo</button>
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Đóng</button>
            </div>
...
<script type="text/javascript">
    jQuery(document).ready(function() {
        ...
        function loadDsCongViecCanXuLyV2(_token){                
            var xhr1;   
            var url="{{ route('loadDsCongViecCanXuLyV2') }}";
        ...
        */
        ...
        jQuery('.js-chuyen-ld-duyet').on('click',function(){
            var _token=jQuery('form[name="frm_chuyen_lanh_dao_duyet"]').find("input[name='_token']").val();
            var idCongViec=jQuery(this).attr('data');
            jQuery('.id-cong-viec').val(idCongViec);
        });


        jQuery('.btn-chuyen-lanh-dao').on('click',function(){
            ...
            var form = $("form#frm_chuyen_lanh_dao_duyet");
            var formData = new FormData(form[0]);
            $.ajax({
                type: "POST",
                url: "{{ route('chuyenLanhDaoDuyetCongViecV2') }}",
                data: formData,
                contentType: false,
                processData: false,
                error:function(){
                ...
                success:function(data){
                    if(data.error==0){                            
                        ...
                        loadDsCongViecCanXuLyV2(_token);
                    }
                    else{
                        $.toast({
                            heading: 'Lỗi!',
        ...
        jQuery('.js-yc-ho-tro').on('click',function(){
            var _token=jQuery('form[name="frm_yeu_cau_ho_tro"]').find("input[name='_token']").val();
            var idCongViec=jQuery(this).attr('data');
            jQuery('.id-cong-viec').val(idCongViec);
        });


        jQuery('.btn-yeu-cau-ho-tro').on('click',function(){
            var noiDungYeuCau=jQuery('#noi_dung_yeu_cau').val();
            if(noiDungYeuCau==''){
                $.toast({
                    heading: 'Lỗi!',
                    text: 'Vui lòng kiểm tra lại dữ liệu.',
            ...
            var form = $("form#frm_yeu_cau_ho_tro");
            var formData = new FormData(form[0]);
            $.ajax({
                type: "POST",
                url: "{{ route('chuyenLanhDaoHoTroCongViecV2') }}",
                data: formData,
                contentType: false,
                processData: false,
                error:function(){
        ...
        function baoCaoHoanTatCongViecV2(_token, idCongViec){                
            var xhr1;   
            var url="{{ route('baoCaoHoanTatCongViecV2') }}";
            if(xhr1 && xhr1.readyState != 4){
                xhr1.abort(); //huy lenh ajax truoc do
            }
            xhr1 = $.ajax({
        ...
        jQuery('.js-bao-cao-hoan-tat').on('click',function(){
            var idCongViec=jQuery(this).attr('data');
            var _token=jQuery('form[name="frm_yeu_cau_ho_tro"]').find("input[name='_token']").val();
            swal({
                title: 'Bạn thật sự muốn cập nhật lại trạng thái xử lý của mình?',
                text: "Nhấn tiếp tục nếu đồng ý cập nhật trạng thái",
                type: 'warning',
                ...
                baoCaoHoanTatCongViecV2(_token, idCongViec)               
            }, function (dismiss) {
                if (dismiss === 'cancel') {
                    swal(
                        'Đã hủy!',
