        ...
        @foreach($dsCongViecs as $congViec)
            ...
            <tr style="cursor: pointer;" data="{{$congViec->id}}" class="js-xem-cay-xu-ly <?php if($stt==1){echo 'active';} ?>">
                ...
                <td class="text-center">
                    <button type="button" data="{{$congViec->id}}"  data-toggle="modal" data-target=".bs-xu-ly-yeu-cau-ho-tro-modal-lg" class="btn btn-gradient waves-effect waves-light js-xu-ly-yeu-cau-duyet"  data-toggle="tooltip" title="Xử lý yêu cầu duyệt"><i class="dripicons-swap"></i></button>
...
<div class="modal fade bs-xu-ly-yeu-cau-ho-tro-modal-lg" tabindex="-1" role="dialog" aria-labelledby="modalXuLyYeuCauHoTro" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-lg center">
        <form class="form-horizontal" role="form" name="frm_xu_ly_yeu_cau_duyet" id="frm_xu_ly_yeu_cau_duyet">
        {{ csrf_field() }}
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h5 class="modal-title" id="myLargeModalLabel">DUYỆT CÔNG VIỆC</h5>
            </div>
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-xs-12 col-md-12">
                        ...
                        <label for="noi_dung_xu_ly">Ý kiến <span class="text-danger"></span></label>
                        <textarea rows="5" class="form-control noi-dung-xu-ly" name="noi_dung_xu_ly" id="noi_dung_xu_ly"></textarea>

                        <div class="form-group">
                            <label for="file_xu_ly">Tài liệu, văn bản đính kèm<span class="text-danger"></span></label>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary btn-tra-loi-yeu-cau"><i class="dripicons-alarm"></i> Chưa đạt, chuyển thực hiện lại</button>
...
<script type="text/javascript">
    jQuery(document).ready(function() {
        ...
        function loadDsCongViecCanLanhDaoDuyetV2(_token){                
            var xhr1;   
            var url="{{ route('loadDsCongViecCanLanhDaoDuyetV2') }}";
        ...
        */
        ...
        jQuery('.js-xu-ly-yeu-cau-duyet').on('click',function(){
            var _token=jQuery('form[name="frm_xu_ly_yeu_cau_duyet"]').find("input[name='_token']").val();
            var idCongViec=jQuery(this).attr('data');
        ...
        jQuery('.btn-tra-loi-yeu-cau').on('click',function(){
            ...
            var form = $("form#frm_xu_ly_yeu_cau_duyet");
            var formData = new FormData(form[0]);
            $.ajax({
                type: "POST",
                url: "{{ route('phanHoiYeuCauDuyetV2') }}",
                data: formData,
                contentType: false,
                processData: false,
                error:function(){
                ...
                success:function(data){
                    if(data.error==0){                            
                        ...
                        loadDsCongViecCanLanhDaoDuyetV2(_token);
                    }
                    else{
                        $.toast({
                            heading: 'Lỗi!',
        ...
        jQuery('.btn-duyet-hoan-thanh-cong-viec').on('click',function(){
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('dongYDuyetCongViecV2') }}",
                data: formData,
                contentType: false,
                processData: false,
                error:function(){
